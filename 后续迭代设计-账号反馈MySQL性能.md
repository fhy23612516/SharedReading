...
8. 登录用户导入书籍，导入内容可用于创建共读房间。
9. 阅读页支持字体、字号、行距、主题、划线和书签标签。
10. 密码找回基础能力：注册时生成恢复码，忘记密码时用恢复码重置密码，登录后可重新生成恢复码。
11. 管理脚本 `tools/reset-password.js` 支持服务器本地给旧账号重置密码并生成恢复码。
...
6. 密码恢复码只保存 `password_recovery_hash`，不保存明文。
7. 注册、重置密码、重新生成恢复码时只向用户返回一次明文恢复码。
8. 使用恢复码重置密码后，旧恢复码和旧登录会话都会失效。

后续生产增强可以考虑登录限流、验证码、邮箱/短信验证，以及按需切换到 `argon2`。
...
5. 退出登录后 token 失效。
6. 注册后返回恢复码。
7. 用户可以用账号、恢复码和新密码重置密码。
8. 登录用户可以重新生成恢复码。
9. 管理员可以在服务器本地用 `tools/reset-password.js` 给没有恢复码的旧账号重置密码。
