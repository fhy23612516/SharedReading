...
5. 密码找回使用 `users.password_recovery_hash` 保存恢复码哈希，不保存恢复码明文。
...
如果你之前已经初始化过旧表结构，也可以重新执行这一句。脚本使用 `CREATE TABLE IF NOT EXISTS`，会补建新增的 `books`、`bookshelf_items`、`reading_history`、`story_comments` 表，并会给旧的 `users` 表补上 `password_recovery_hash` 字段，不会删除现有数据。
